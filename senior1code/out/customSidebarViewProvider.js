...
class CustomSidebarViewProvider {
    constructor(_extensionUri) {
        ...
        this._prompts = [];
        this._defaultPrompt = `Generate Google-style documentation for this code. Include:
...
5. Performance considerations`
    ...
    async handleGenerateDocs(customPrompt) {
    ...
    async loadSavedPrompts() {
        const userPrompts = await vscode.commands.executeCommand('vscodeSidebar.getSavedPrompts') || [];
        return [...this._predefinedPrompts, ...userPrompts];
    }
    async handleSavePromptRequest(content) {
        try {
            ...
            if (existing) {
                const overwrite = await vscode.window.showQuickPick(['Overwrite', 'Cancel'], { placeHolder: 'Prompt "' + name + '" exists. Overwrite?' });
            ...
            await vscode.commands.executeCommand('vscodeSidebar.savePrompts', this._prompts.filter(p => !p.name.startsWith('[System]')));
            this.sendPromptsToWebview();
    ...
    async handleDeletePromptRequest(name) {
        try {
